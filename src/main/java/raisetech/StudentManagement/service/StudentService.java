 ...
 */
  ...
  /**
   * 受講生検索です。 IDに紐付く受講生情報を取得した後、その受講生に紐付く受講生コース情報を取得して設定します。
   *
   * @param id 受講生ID
   * @return　受講生
   */
  ...
  /**
   * 受講生詳細の登録を行います。 受講生IDが新規作成されます。（その時点でのMAXに+1） 受け取った受講生情報を登録します。 受講生に紐付いたコース情報（値や日付情報）を登録します。
   *
  ...
  /**
   * 受講生詳細の情報を更新します。 キャンセルせずコース情報を変更する場合は、受講生IDに紐付く受講コース情報を更新します。
   * キャンセルせず新規にコース情報を登録する場合も、受講生IDに紐付く受講コース情報を更新します。 受講生がキャンセルされる場合は、受講生IDに紐付く受講コース情報もキャンセルします。
   *
  ...
  public void updateStudent(StudentDetail studentDetail) {
    ...
    if (studentDetail.getStudentsCourseList() == null
        || studentDetail.getStudentsCourseList().isEmpty()) {
      return;
    }
      ...
      if (newCourse.getCourseName() == null || newCourse.getCourseName().isBlank()) {
        return;
      }
